...
### Added
- Shareable results link (v0.2 Feature 5): a "Copy shareable link" button on the results screen (shown only in browsers that support it) encodes the source label, trust scores, reasons, and each metric's chart history as gzip-compressed, base64url JSON in the URL fragment (`#s=...`) - no backend, nothing server-side, matching the app's existing fully-client-side architecture. Opening a link with that fragment decodes it and renders the results directly, with no re-analysis and no need for the original file; a corrupted or unrecognized fragment shows a clear error instead of failing silently. The fragment is cleared from the address bar once handled, so refreshing or navigating away doesn't replay it. Uses the browser's native CompressionStream/DecompressionStream - no new dependency.
